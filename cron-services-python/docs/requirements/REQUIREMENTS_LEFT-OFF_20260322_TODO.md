...
- [ ] Review `/Users/nick/Documents/_project_resources/PersonalWeb03/obsidian/LEFT-OFF.md` and confirm the file matches the agreed structure.
- [ ] Confirm date headings use only `# YYYYMMDD`.
- [ ] Confirm `#` headings are date headings only, with no non-date top-level headings.
- [ ] Confirm the file is ordered newest first and oldest last.
- [ ] Confirm all content for a date lives under that date heading until the next `# YYYYMMDD` heading.
- [ ] Confirm currently supported markdown content includes plain text, `- [ ]` task items, and backtick code snippets.
...
- [ ] Update the LEFT-OFF service configuration so it no longer requires OneDrive credentials or file IDs.
- [ ] Add a path helper for the new markdown source file at `PATH_PROJECT_RESOURCES/obsidian/LEFT-OFF.md`.
- [ ] Keep the existing output path for `last-7-days-activities.md` unless there is a strong reason to change it.
- [ ] Update the LEFT-OFF execution flow in `src/main.py` to remove the OneDrive download step.
- [ ] Replace the old three-step logging with the new local-file flow so logs reflect the actual process.
- [ ] Remove any imports and setup code that are no longer needed once OneDrive is no longer part of the LEFT-OFF flow.
- [ ] Commit changes for Phase 2 before moving to Phase 3.

## Phase 3. Replace the parser

- [ ] Replace the `.docx` parser implementation with markdown-based parsing for `LEFT-OFF.md`.
- [ ] Preserve the current 7-day behavior by finding the first `# YYYYMMDD` heading that is 8 or more days old and stopping extraction there.
- [ ] Treat every line under a `# YYYYMMDD` heading as part of that day until the next top-level date heading.
- [ ] Preserve markdown content when writing `last-7-days-activities.md`.
- [ ] Ensure unchecked task items such as `- [ ]` pass through without being altered.
- [ ] Ensure inline code and fenced code content are preserved as written.
- [ ] Make the parser tolerant of blank lines and normal markdown spacing.
...
- [ ] Update the LEFT-OFF summarizer prompt so it no longer says the source text came from a Microsoft Word document.
- [ ] Update README output and workflow descriptions to reflect Obsidian markdown as the source.
- [ ] Update engineering notes and requirements references that still describe OneDrive and `.docx` as the active flow.
- [ ] Keep archived requirements untouched unless there is a clear reason to annotate them as historical.
- [ ] Review any comments, help text, and CLI descriptions that still mention `LEFT-OFF.docx`.
