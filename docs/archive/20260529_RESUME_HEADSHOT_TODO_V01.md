...
- [ ] Add `useState` to the existing React import (the file is already a client component but does not currently import `useState`).
- [ ] Import `ModalDisplayPicture` from `@/components/ModalDisplayPicture` (match the import style used by `HeroSection.tsx`).
...
- [ ] Add the thumbnail trigger inside the new left wrapper:
  ...
  - Use `next/image`'s `<Image>` (already used elsewhere in the project).
...
- [ ] `npm run lint` — must pass with no new warnings (watch for the new `useState` import and a11y on the button).
