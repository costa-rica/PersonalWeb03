...
- [x] Confirm the source asset exists at `/home/nick/.hermes/image_cache/img_52f85da30cd1.jpg` (JPEG, 800x800). If missing, stop and ask the operator.
- [x] Copy the source byte-for-byte to `web/public/headshotNRodriguez.jpg` (used by the modal — must remain unmodified).
- [x] Copy the operator-supplied head-and-shoulders image from `/home/nick/.hermes/image_cache/img_b7d64388f2d4.jpg` to `web/public/headshotNRodriguezCrop.jpg`:
...
- [x] Visually open both files and confirm:
...
- [x] Re-crop if the framing is off. The exact offsets are not load-bearing; the visual result is. No re-crop was needed because the operator supplied the dedicated shoulder/headshot asset.

### Phase 1 verification

- [x] `ls web/public/headshotNRodriguez.jpg web/public/headshotNRodriguezCrop.jpg` both exist.
- [x] `file web/public/headshotNRodriguez.jpg` and `file web/public/headshotNRodriguezCrop.jpg` both report JPEG image data.
...
- [x] Add the following three imports at the top of the file, alongside the existing `next/link` and `lucide-react` imports (the file currently has **no React import and no `next/image` import**; do not assume one exists):
...
- [x] Add modal state inside the component:

  ```ts
  const [isHeadshotModalOpen, setIsHeadshotModalOpen] = useState(false);
  ```

- [x] Restructure the "Top Personal Info Section" left column (current `flex flex-col lg:flex-row gap-6 mb-8` container around lines 115–195) so the headshot sits to the left of the name / title / objective stack:
...
- [x] Add the thumbnail trigger inside the new left wrapper:
...
- [x] Render `<ModalDisplayPicture />` near the bottom of the returned JSX (outside the layout flex containers but inside the section root, matching `HeroSection.tsx`'s usage around lines 228–233):
...
- [x] Do not introduce a constant, helper, or new shared component for the headshot. Keep paths inline.
- [x] Do not modify the existing Download PDF button, contact column, or any other resume content.

### Phase 2 verification

Run from `web/`:

- [x] `npm run lint` — must pass with no new warnings (watch for unused imports and a11y on the button). Passed with existing unrelated warnings and no `ResumeSection.tsx` warnings.
- [x] `npm run build` — must succeed. (`next.config.mjs` has `ignoreBuildErrors: true` for TS per `web/AGENTS.md`, so lint is the stronger TS signal; the build should still complete cleanly.) Passed.
- [x] No automated test suite is configured for `web/` per `web/AGENTS.md`. Skip test execution unless one has been added since.
- [x] Manual browser smoke test with `npm run dev`:
  - [x] Resume section renders the cropped thumbnail to the left of the name on desktop widths.
  - [x] The visible thumbnail shows head and shoulders only — no torso, no full body, no awkward face cut-off.
  - [ ] On narrow widths the layout stacks without overlap or overflow.
  - [x] Clicking the thumbnail opens `ModalDisplayPicture` at ~90vw × 90vh and shows the **full uncropped original**.
  - [x] Escape and backdrop click both close the modal; body scroll is restored after close.
  - [x] Keyboard: Tab to the thumbnail, press Enter / Space to open the modal; Escape closes. Verified Enter + Escape after focusing the thumbnail.
...
- [ ] Check off the completed Phase 1 + Phase 2 tasks and commit. Use an `ideal` `feat:` commit per `AGENTS.md` (title ≤ 50 chars, e.g. `feat: add clickable headshot to resume section`).
